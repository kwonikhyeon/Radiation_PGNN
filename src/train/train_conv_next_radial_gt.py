...
# 단순화된 모델 임포트
try:
    from model.radial_conv_next_pgnn import (
...
def train_one_epoch_simple_gt(model, loader, optimizer, epoch, cfg):
    ...
    for batch_idx, (inp, gt, names) in enumerate(pbar):
        ...
        # 4.5. 원형 물리 손실 (새로 추가)
        radial_loss = torch.tensor(0.0, device=cfg.device)
        if epoch >= 10 and cfg.enable_radial_guide:  # 10 에포크 후 적용
            try:
                radial_loss = radial_physics_loss(pred, inp[:, 0:1], inp[:, 1:2])
                
                # 안전 장치
                if torch.isnan(radial_loss) or torch.isinf(radial_loss):
                    radial_loss = torch.tensor(0.0, device=cfg.device)
                elif radial_loss > 2.0:
                    radial_loss = torch.clamp(radial_loss, max=2.0)
            except Exception as e:
                radial_loss = torch.tensor(0.0, device=cfg.device)
        ...
        loss = (
            ...
            cfg.radial_weight * radial_loss +                 # 원형 물리 손실 (새로 추가)
        ...
        pbar.set_postfix({
            ...
            "RadialW": f"{cfg.radial_weight:.4f}",
            "Radial": f"{radial_loss.item():.4f}",
...
class SimpleGTTrainConfig:
    def __init__(self, **kwargs):
        ...
        # 원형 가이드 설정
        self.enable_radial_guide = kwargs.get('enable_radial_guide', True)
        self.radial_weight = kwargs.get('radial_weight', 0.1)
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints/simple_gt_radial_exp1")
    ...
    # 원형 가이드 전용 옵션
    parser.add_argument("--enable_radial_guide", action='store_true', default=True,
                       help="Enable radial bias guide layer")
    parser.add_argument("--radial_weight", type=float, default=0.1,
                       help="Radial physics loss weight")
    ...
    cfg = SimpleGTTrainConfig(
        ...
        enable_radial_guide=args.enable_radial_guide,
    ...
    print(f"🎯 Radial Guide: {cfg.enable_radial_guide}")
    if cfg.enable_radial_guide:
        print(f"   - Weight: {cfg.radial_weight}")
    ...
    config_dict = {
        ...
        'enable_radial_guide': cfg.enable_radial_guide,
        'radial_weight': cfg.radial_weight,
